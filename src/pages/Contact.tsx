...
export const Contact: React.FC = () => {
  ...
  const [activeRoute, setActiveRoute] = useState<'us' | 'eu' | 'me' | 'asia'>('us');

  const routeStats = {
    us: { region: 'North America (LA / NY Ports)', transit: '18-24 Days', temp: 'Sub-Zero Cold Chain / Ambient', capacity: 'Weekly FCL' },
    eu: { region: 'Europe (Rotterdam / Hamburg)', transit: '28-35 Days', temp: 'Sub-Zero Cold Chain / Controlled Atmosphere', capacity: 'Bi-weekly FCL' },
    me: { region: 'Middle East (Jebel Ali)', transit: '12-15 Days', temp: 'Ambient / Cold Chain', capacity: 'Weekly FCL' },
    asia: { region: 'East Asia (Tokyo / Busan)', transit: '5-8 Days', temp: 'Ambient / Cold Chain', capacity: 'Multi-weekly FCL' },
  };
  ...
  return (
    <div className="font-sans overflow-hidden bg-cream text-carbon">
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center bg-carbon text-cream pt-32 pb-16 px-4 md:px-8">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10 flex flex-col items-center gap-6">
          ...
          >
            ...
            <h1 className="font-serif text-[2.75rem] leading-[1.1] sm:text-[3.5rem] md:text-6xl lg:text-7xl font-bold leading-tight tracking-wide text-gold-champagne"><span dangerouslySetInnerHTML={{ __html: t('contact.heroTitle') }} /></h1>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-5 flex flex-col gap-10">
            <div className="flex flex-col gap-4">
              ...
              <h2 className="font-serif text-[2.5rem] leading-[1.1] sm:text-5xl md:text-6xl font-bold text-forest"><span dangerouslySetInnerHTML={{ __html: t('contact.introTitle') }} /></h2>
      ...
      {/* Global Trade Route Map Visualisation */}
      <section className="py-24 bg-ivory border-t border-gold-warm/15 px-4 md:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center flex flex-col gap-4 max-w-3xl mx-auto mb-16">
            ...
            <h2 className="font-serif text-[2.5rem] leading-[1.1] sm:text-5xl md:text-6xl font-bold text-forest">
              {t('contact.globalReady')}
            </h2>
            <p className="text-sm text-carbon/75 font-light leading-relaxed">
              {t('contact.globalReadyDesc')}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            {/* Interactive SVG Map */}
            <div className="lg:col-span-8 bg-cream border border-gold-warm/15 rounded-2xl p-6 md:p-12 shadow-sm flex items-center justify-center relative min-h-[400px]">
              
              {/* World outline SVG */}
              <svg viewBox="0 0 950 620" className="w-full h-auto">
                {/* Detailed World Map Background */}
                <image href="/world.svg" width="950" height="620" className="opacity-40" />
                ...
                {/* HCMC to Middle East Route */}
                <path 
                  d="M 754,275 Q 687,200 620,223" 
                  fill="none" 
                  stroke="#104D2E"
                ...
                {/* HCMC to East Asia Route */}
                <path 
                  d="M 754,275 Q 797,180 840,190" 
                  fill="none" 
                  stroke="#104D2E"
              ...
              {/* Pulsing connection label */}
              <div className="absolute bottom-4 left-4 bg-forest text-cream text-[10px] uppercase font-bold tracking-widest px-3 py-1.5 rounded-full border border-gold-warm/20 flex items-center gap-1.5 shadow-sm">
                <span className="w-2 h-2 rounded-full bg-gold-warm animate-ping" />
                Ho Chi Minh City Port Hub
              </div>
            </div>

            {/* Route Details Panel */}
            <div className="lg:col-span-4 flex flex-col gap-6">
              <h3 className="font-serif text-2xl font-bold text-forest border-b border-gold-warm/25 pb-3">
                Trade Route Details
              </h3>

              <div className="flex flex-col gap-2">
                <span className="text-[10px] uppercase tracking-wider text-carbon/60 font-bold">Select Active Route</span>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: 'us', label: 'North America' },
                    { id: 'eu', label: 'Europe' },
                    { id: 'me', label: 'Middle East' },
                    { id: 'asia', label: 'East Asia' },
                  ].map(btn => (
                    <button
                      key={btn.id}
                      onClick={() => setActiveRoute(btn.id as any)}
                      className={`px-3 py-2 text-xs font-semibold rounded border transition-all duration-200 cursor-pointer ${
                        activeRoute === btn.id 
                          ? 'bg-forest text-cream border-forest' 
                          : 'bg-cream text-carbon border-gold-warm/25 hover:border-gold-warm'
                      }`}
                    >
                      {btn.label}
                    </button>
              ...
              <motion.div
                ...
                className="bg-cream border border-gold-warm/15 rounded-xl p-6 flex flex-col gap-4 shadow-sm"
              >
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-gold-antique font-bold">Destination Region</span>
                  <span className="text-base font-serif font-bold text-forest mt-0.5">{routeStats[activeRoute].region}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-gold-antique font-bold">Avg. Transit Duration</span>
                  <span className="text-sm font-semibold text-carbon mt-0.5">{routeStats[activeRoute].transit}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-gold-antique font-bold">Cold Chain Standard</span>
                  <span className="text-sm font-semibold text-carbon mt-0.5">{routeStats[activeRoute].temp}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] uppercase tracking-wider text-gold-antique font-bold">Departure Frequency</span>
                  <span className="text-sm font-semibold text-carbon mt-0.5">{routeStats[activeRoute].capacity}</span>
                </div>
              </motion.div>
            </div>
